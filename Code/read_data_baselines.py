def read_data(): #English binary sentiment analysis task
    df = pd.read_csv("../Data/imdb_movie_reviews.csv", names=["text", "labels"])
    train_df = df[1:25001]
...
def read_data(): #Dutch binary sentiment analysis
    train_df = pd.read_csv("../Data/dutch_book_reviews_train.csv", names=["text", "labels"])
    test_df = pd.read_csv("../Data/dutch_book_reviews_test.csv", names=["text", "labels"])
...
def read_data(): #5 star multiclass sentiment analysis
    ...
    text = []
    labels = []
    df = pd.read_csv("../Data/five_star_movie_reviews.tsv",sep="\t", names=["PhraseId","SentenceId","Phrase","Sentiment"],low_memory=False)
